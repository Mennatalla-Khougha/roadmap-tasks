...
from services.user_services import create_user, get_user, user_login, add_roadmap_to_user, get_user_roadmap, \
    get_user_roadmaps, update_user_roadmap, delete_user_roadmap, delete_all_user_roadmaps
...
def get_user_endpoint(current_user_token: TokenData = Depends(get_current_user)):
    """
    Endpoint to get a user by email.
    This endpoint retrieves user details based on the provided email.
    Args:
        current_user_token (TokenData): The current user's token data, which includes the email.
    Raises:
...
async def add_roadmaps_to_user_endpoint(roadmap_id: str, current_user_token: TokenData = Depends(get_current_user)):
    """
...
async def get_user_roadmaps_endpoint(current_user_token: TokenData = Depends(get_current_user)):
    """
...
async def get_user_roadmap_endpoint(roadmap_id: str, current_user_token: TokenData = Depends(get_current_user)):
    """
    Endpoint to get all roadmaps of a user.
    This endpoint retrieves the list of roadmap IDs associated with the user.
    Args:
...
async def update_user_roadmap_endpoint(roadmap_id: str, updated_fields: dict, current_user_token: TokenData = Depends(get_current_user)):
    """
...
async def delete_user_roadmap_endpoint(roadmap_id: str, current_user_token: TokenData = Depends(get_current_user)):
    """
    ...
    Args:
        ...
        current_user_token (TokenData): The current user's token data, which includes the email.
    Raises:
        HTTPException: If the user or roadmap is not found, or if there is an unexpected error.
    Returns:
...
async def delete_all_user_roadmaps_endpoint(current_user_token: TokenData = Depends(get_current_user)):
    """
