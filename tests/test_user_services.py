...
def test_create_user_with_valid_data(mock_db, mock_hash_password):
    ...
    assert result.id == user_data.email # As per create_user logic
...
def test_get_user_with_valid_email(mock_db):
    ...
    mock_db.collection.return_value.document.return_value.get.return_value = mock_user_get_result
...
def test_user_login_with_empty_password(mock_db): # Renamed for clarity
    # Arrange
    user_login_data = UserLogin(email="test@example.com", password="") # Valid email, empty password

    # Act & Assert
    with pytest.raises(ValueError) as exc_info:
        user_login(user_login_data)
    assert "Error logging in with email or password: Email and password are required" in str(exc_info.value)
...
def test_user_login_with_empty_credentials(mock_db):
    # Pydantic's EmailStr prevents email="", so we test with a valid email and empty password
    # to hit the service's internal check for empty credentials.
...
def test_user_login_with_nonexistent_email(mock_db):
    ...
    with pytest.raises(ValueError) as exc_info:
        user_login(user_login_data)
    assert "Error logging in with email or password: Invalid password or email" in str(exc_info.value)
...
def test_user_login_with_incorrect_password(mock_db, mock_verify_password):
    ...
    user_data = {"email": "test@example.com", "password": "hashedpassword", "id": "someid", "role": "user"}
    mock_user_get_result = MagicMock()
    mock_user_get_result.exists = True
    mock_user_get_result.to_dict.return_value = user_data
    mock_db.collection.return_value.document.return_value.get.return_value = mock_user_get_result
...
async def test_add_roadmap_to_user_success(mock_db, mock_get_user_service, mock_get_roadmap_service,
    ...
    with patch("services.user_services.asyncio.to_thread", new_callable=AsyncMock) as mock_to_thread:
        # Act
...
async def test_add_roadmap_to_user_already_exists(mock_get_user_service, sample_user_response):
    ...
    user_with_roadmap.user_roadmaps_ids = [roadmap_id_val] # Roadmap already in user's list
...
async def test_add_roadmap_to_user_get_roadmap_fails(mock_get_user_service, mock_get_roadmap_service,
    ...
    email = "test@example.com"
    roadmap_id = "roadmap1"
    mock_get_user_service.return_value = sample_user_response # User exists and has no roadmaps initially
...
async def test_get_user_roadmaps_success(mock_get_user_service, sample_user_response, sample_roadmap):
    ...
    with patch("services.user_services.get_user_roadmap", new_callable=AsyncMock) as mock_get_user_roadmap_specific:
        mock_get_user_roadmap_specific.return_value = sample_roadmap # It will return this for each ID
...
async def test_get_user_roadmaps_get_specific_roadmap_fails(mock_get_user_service, sample_user_response):
    ...
    with patch("services.user_services.get_user_roadmap", new_callable=AsyncMock) as mock_get_user_roadmap_specific:
        mock_get_user_roadmap_specific.side_effect = Exception("Failed to fetch specific roadmap")

        # Act & Assert
        with pytest.raises(Exception) as exc_info:
            await get_user_roadmaps(email)
        assert "Unexpected Error: Failed to fetch specific roadmap" in str(exc_info.value)
...
async def test_get_user_roadmap_success(mock_db, mock_get_user_service, mock_fetch_roadmap_from_firestore_util,
    ...
    roadmap_id = "roadmap1"
    user_with_roadmap = UserResponse(**sample_user_response.model_dump())
    user_with_roadmap.user_roadmaps_ids = [roadmap_id] # User has this roadmap ID
    mock_get_user_service.return_value = user_with_roadmap
    mock_fetch_roadmap_from_firestore_util.return_value = sample_roadmap

    # Mock for doc_ref = db.collection("users").document(email).collection("users_roadmaps")
    mock_users_roadmaps_collection_ref = MagicMock(name="users_roadmaps_coll_ref_get_user_roadmap")
    mock_db.collection.return_value.document.return_value.collection.return_value = mock_users_roadmaps_collection_ref
    ...
    mock_fetch_roadmap_from_firestore_util.assert_called_once_with(mock_users_roadmaps_collection_ref, roadmap_id)
...
async def test_get_user_roadmap_not_in_users_list(mock_get_user_service, sample_user_response):
    ...
    user_without_this_roadmap = UserResponse(**sample_user_response.model_dump())
    user_without_this_roadmap.user_roadmaps_ids = ["another_roadmap_id"] # User has other roadmaps
...
async def test_get_user_roadmap_fetch_fails(mock_db, mock_get_user_service, mock_fetch_roadmap_from_firestore_util,
    ...
    mock_users_roadmaps_collection_ref = MagicMock(name="users_roadmaps_coll_ref_fetch_fails")
    mock_db.collection.return_value.document.return_value.collection.return_value = mock_users_roadmaps_collection_ref
...
def test_get_user_with_empty_email(mock_db): # No mock_get_user_service needed as it's testing get_user directly
    # Arrange
    email = ""
...
@pytest.mark.asyncio
async def test_update_user_roadmap_success(mock_db, mock_get_user_service, sample_user_response):
    # Arrange
    email = "test@example.com"
    roadmap_id = "roadmap1"
    ...
    mock_doc_snapshot = MagicMock(name="firestore_doc_snapshot_update_success")
    ...
    with patch("services.user_services.asyncio.to_thread", side_effect=mock_async_to_thread_executor) as mock_async_to_thread:
...
@pytest.mark.asyncio
async def test_update_user_roadmap_id_missing(mock_get_user_service, sample_user_response):
    email = "test@example.com"
    ...
    with pytest.raises(ValueError) as exc_info:
        await update_user_roadmap(roadmap_id, updated_fields, email)
    assert "Invalid input or operation: Roadmap ID is required" in str(exc_info.value)


@pytest.mark.asyncio
async def test_update_user_roadmap_not_in_users_list(mock_get_user_service, sample_user_response):
    email = "test@example.com"
    roadmap_id = "roadmap_not_owned"
    updated_fields = {"title": "New Title"}
    user_without_roadmap = UserResponse(**sample_user_response.model_dump())
    user_without_roadmap.user_roadmaps_ids = ["some_other_roadmap"] # User doesn't have roadmap_id
    mock_get_user_service.return_value = user_without_roadmap
    with pytest.raises(RoadmapNotFoundError) as exc_info:
        await update_user_roadmap(roadmap_id, updated_fields, email)
    assert "Roadmap not found: Roadmap not found in user's roadmaps" in str(exc_info.value)


@pytest.mark.asyncio
async def test_update_user_roadmap_doc_not_exists_in_firestore(mock_db, mock_get_user_service, sample_user_response):
    email = "test@example.com"
    roadmap_id = "roadmap1"
    updated_fields = {"title": "New Title"}
    user_with_roadmap = UserResponse(**sample_user_response.model_dump())
    user_with_roadmap.user_roadmaps_ids = [roadmap_id] # User owns the roadmap ID
    mock_get_user_service.return_value = user_with_roadmap
    ...
    mock_doc_ref.get = MagicMock(return_value=mock_doc_snapshot)
    mock_doc_ref.update = MagicMock(name="firestore_doc_ref_update_method_not_exists")
    ...
    assert f"Roadmap not found: Roadmap with id {roadmap_id} not found for user {email}" in str(exc_info.value)
...
@pytest.mark.asyncio
async def test_update_user_roadmap_no_valid_fields(mock_get_user_service, sample_user_response):
    email = "test@example.com"
    roadmap_id = "roadmap1"
    user_with_roadmap = UserResponse(**sample_user_response.model_dump())
    user_with_roadmap.user_roadmaps_ids = [roadmap_id] # User must have the roadmap
    mock_get_user_service.return_value = user_with_roadmap

    with pytest.raises(ValueError) as exc_info:
        await update_user_roadmap(roadmap_id, {"invalid_field": "value", "another_bad": 123}, email)
    assert "Invalid input or operation: No valid fields provided for update or all fields are disallowed." in str(
        exc_info.value)